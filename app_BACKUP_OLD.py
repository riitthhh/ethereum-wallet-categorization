...
Integrated Flask Backend + Frontend Server
JUST RUN THIS FILE - Everything works automatically!

FIXED ISSUES:
1. OrderedDict model loading - now creates SimpleNeuralNetwork
2. Statistics error when no predictions - proper None handling
3. Added sample test CSV generation
"""
...
# Wallet categories (matching training)
WALLET_CATEGORIES = [
    'Exchange Wallet',
...
class ModelLoader:
    ...
    def load_model(model_dir: str):
        """Load PyTorch/TensorFlow model from directory"""
        try:
            ...
            try:
                ...
                if isinstance(loaded_obj, dict):
                    ...
                    if 'model_state_dict' in loaded_obj:
                        ...
                        # Assume it's a raw state dict (OrderedDict)
                        state_dict = loaded_obj
                        logger.info("Treating as raw state dict")
                    ...
                    logger.info("Created SimpleNeuralNetwork")
                    ...
                    logger.warning(f"Unknown PyTorch object type: {type(loaded_obj)}")
                ...
                try:
                    ...
                    logger.error(f"Joblib loading also failed: {e2}")
                    logger.info("Creating new model with random weights...")
                    model = SimpleNeuralNetwork(input_size=20)
...
class WalletFeatureExtractor:
    """Extract features from wallet addresses (matching training pipeline)"""
    
    @staticmethod
    def extract_features(address: str, blockchain_data: Optional[Dict] = None) -> Dict:
        """
        Extract features from wallet address
        This uses mock data - replace with actual blockchain API in production
        """
        features = {}
        ...
        # Mock blockchain data (replace with real API calls)
        ...
        features['balance_eth_ratio'] = (
            features['current_balance'] / max(features['total_eth_received'], 1)
        )
    ...
    def _get_mock_blockchain_data(address: str) -> Dict:
        ...
        In production, replace with actual Etherscan/Infura API calls
        """
...
class WalletPredictor:
    ...
    def predict(self, address: str) -> Dict:
        ...
        try:
            ...
            if isinstance(self.model, nn.Module):
                ...
                # Scikit-learn model without predict_proba
                ...
                raise ValueError(f"Model type {type(self.model)} not supported for prediction")
    ...
    def _analyze_risk(self, category: str, features: Dict, probabilities: np.ndarray) -> Dict:
        """Analyze wallet risk based on category and features"""
        risk_factors = []
        risk_score = 0
...
def get_sample_csv():
    ...
    try:
        ...
        return send_file(
            csv_path,
            as_attachment=True,
            download_name='sample_wallet_addresses.csv',
            mimetype='text/csv'
        )
    except Exception as e:
...
def initialize_model():
    """Initialize model from provided path"""
...
def predict_batch():
    """Predict batch of addresses from CSV"""
    if MODEL is None:
        return jsonify({'error': 'Model not initialized'}), 400
    ...
    try:
        ...
        for result in results:
            ...
            export_data.append({
                ...
                'ERC-20 Transfers': stats.get('erc20_activity', 0),
                'ERC-721 Transfers': stats.get('erc721_activity', 0),
...
def get_statistics():
    """Get overall statistics - FIXED to handle None values"""
    try:
        conn = sqlite3.connect('data/predictions.db')
        ...
        # Overall stats - Fixed to handle None
...
if __name__ == '__main__':
    ...
    print("\n" + "="*70)
    print("  🚀 CryptoGuard - AI Wallet Intelligence Platform")
    print("="*70)
    ...
    print("   3. Start analyzing wallets!")
    print("\n📁 Sample CSV created: data/sample_addresses.csv")
    print("   You can use this for batch testing!")
